...
function main() {
  ...
  var params =  {paymentId: urlParams.get("paymentId"), payerId: urlParams.get("PayerID")};
  if(!params.paymentId || !params.payerId) {
    disp.show("Invalid Transaction", 1); return;
  }

  /* Make Request */
  disp.show("Finalizing Transaction", 0);
  $.ajax({
    type: "POST",
    url: "../pay/process",
...
function onSuccess() {
  ...
  var autoClose = function() {
    disp.show("Complete", 2, "Returning to main site" + dots);
    cnt++; dots += ".";
