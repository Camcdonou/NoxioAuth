...
Display.prototype.setupWebGL = function() {
  ...
  if(!this.createMaterial(this.game.asset.material.multi.debug)) { return false; }
...
Display.prototype.compileVertexShader =  function(name, source) {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.log("An error occurred compiling the Vertex Shader: " + name); console.log(gl.getShaderInfoLog(shader));
    return undefined;
  }
  return shader;
};
...
Display.prototype.compileFragmentShader =  function(name, source) {
  ...
  // See if it compiled successfully
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.log("An error occurred compiling the Fragment Shader: " + name); console.log(gl.getShaderInfoLog(shader));
    return undefined;
  }
  return shader;
};
...
Display.prototype.createShader = function(source) {
  ...
  if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) {
    console.log("Unable to initialize the shader program: "); console.log(gl.getProgramInfoLog(shaderProgram));
    return false;
  }
...
Display.prototype.draw = function() {
  ...
  // @TODO: need to update shadow framebuffer possibly...
  ...
  var skyMaterial = this.getMaterial("multi.debug");
  ...
  var grid = {
    border: [
      new Line(gl, {
        a: {x: 0.0, y: 0.0},
        b: {x: this.game.map.size.x, y: 0.0}
      }),
      new Line(gl, {
        a: {x: this.game.map.size.x, y: 0.0},
        b: {x: this.game.map.size.x, y: this.game.map.size.y}
      }),
      new Line(gl, {
        a: {x: this.game.map.size.x, y: this.game.map.size.y},
        b: {x: 0.0, y: this.game.map.size.y}
      }),
      new Line(gl, {
        a: {x: 0.0, y: this.game.map.size.y},
        b: {x: 0.0, y: 0.0}
      })
    ],
    grid: new Grid(gl, this.game.map.size)
  };
  if(this.game.hover && this.game.settings.cursor === 0) {
    var hpc = this.game.hover;
    grid.hover = [
      new Line(gl, {
        a: {x: hpc.x, y: hpc.y},
        b: {x: hpc.x+1.0, y: hpc.y}
      }),
      new Line(gl, {
        a: {x: hpc.x+1.0, y: hpc.y},
        b: {x: hpc.x+1.0, y: hpc.y+1.0}
      }),
      new Line(gl, {
        a: {x: hpc.x+1.0, y: hpc.y+1.0},
        b: {x: hpc.x, y: hpc.y+1.0}
      }),
      new Line(gl, {
        a: {x: hpc.x, y: hpc.y+1.0},
        b: {x: hpc.x, y: hpc.y}
      })
    ];
  }
  else if(this.game.hover && this.game.settings.cursor === 1) {
    var hpc = this.game.cursor;
    grid.hover = [
      new Line(gl, {
        a: {x: hpc.x-0.25, y: hpc.y+0.25},
        b: {x: hpc.x+0.25, y: hpc.y-0.25}
      }),
      new Line(gl, {
        a: {x: hpc.x-0.25, y: hpc.y-0.25},
        b: {x: hpc.x+0.25, y: hpc.y+0.25}
  ...
  if(this.game.selection) {
    var hpc = this.game.selection;
    selection = [
      new Line(gl, {
        a: {x: hpc.x-0.1, y: hpc.y-0.1},
        b: {x: hpc.x+0.1, y: hpc.y-0.1}
      }),
      new Line(gl, {
        a: {x: hpc.x+0.1, y: hpc.y-0.1},
        b: {x: hpc.x+0.1, y: hpc.y+0.1}
  ...
  if(this.game.settings.doodad) {
    for(var i=0;i<this.game.map.doodads.length;i++) {
      var dpos = this.game.map.doodads[i].pos;
      doodads.push(new Line(gl, {
        a: {x: dpos.x-0.25, y: dpos.y+0.25},
        b: {x: dpos.x+0.25, y: dpos.y-0.25}
      }));
      doodads.push(new Line(gl, {
        a: {x: dpos.x-0.25, y: dpos.y-0.25},
        b: {x: dpos.x+0.25, y: dpos.y+0.25}
  ...
  if(this.game.settings.spawn) {
    for(var i=0;i<this.game.map.spawns.length;i++) {
      var spos = this.game.map.spawns[i].pos;
      spawns.push(new Line(gl, {
        a: {x: spos.x-0.25, y: spos.y-0.25},
        b: {x: spos.x+0.25, y: spos.y-0.25}
      }));
      spawns.push(new Line(gl, {
        a: {x: spos.x+0.25, y: spos.y-0.25},
        b: {x: spos.x+0.25, y: spos.y+0.25}
  ...
  if(this.game.cameraOriginScrollPoint) {
    grid.cammove = new Line(gl, {a: util.vec2.add(this.game.cameraScrollPoint, {x: 0.5, y: 0.5}), b: util.vec2.add(this.game.cameraOriginScrollPoint, {x: 0.5, y: 0.5})});
  }
  
  var collision = {};
  if(this.game.settings.collision.draw && this.game.settings.collision.floor) {
    collision.floor = [];
    for(var i=0;i<this.game.map.collision.floor.length;i++) {
      for(var j=0;j<this.game.map.collision.floor[i].length-1;j++) {
        collision.floor.push(new Line(gl, {a: this.game.map.collision.floor[i][j], b: this.game.map.collision.floor[i][j+1]}));
      }
      collision.floor.push(new Line(gl, {a: this.game.map.collision.floor[i][j], b: this.game.map.collision.floor[i][0]}));
    }
  }
  if(this.game.settings.collision.draw && this.game.settings.collision.wall) {
    collision.wall = [];
    for(var i=0;i<this.game.map.collision.wall.length;i++) {
      for(var j=0;j<this.game.map.collision.wall[i].length-1;j++) {
        collision.wall.push(new Line(gl, {a: this.game.map.collision.wall[i][j], b: this.game.map.collision.wall[i][j+1]}));
      }
      collision.wall.push(new Line(gl, {a: this.game.map.collision.wall[i][j], b: this.game.map.collision.wall[i][0]}));
    }
  }
  ...
  /* Draw the line overlays */
  var lineMat = this.getMaterial("multi.debug");
  var lineUniformData = [
    {name: "Pmatrix", data: PROJMATRIX},
    {name: "Vmatrix", data: VIEWMATRIX},
    {name: "Mmatrix", data: MOVEMATRIX},
    {name: "transform", data: [-0.5, -0.5, 0.0]}         // Tiles are centered so to put lines on edges we need to offset slightly.
  ];
  var lineRedUniformData = [ {name: "color", data: [1.0, 0.0, 0.0]} ];
  var lineGreenUniformData = [ {name: "color", data: [0.0, 1.0, 0.0]} ];
  var linePinkUniformData = [ {name: "color", data: [1.0, 0.0, 0.5]} ];
  var lineWhiteUniformData = [ {name: "color", data: [1.0, 1.0, 1.0]} ];
  var lineBlueUniformData = [ {name: "color", data: [0.0, 0.0, 1.0]} ];
  var lineYellowUniformData = [ {name: "color", data: [1.0, 1.0, 0.0]} ];
  var lineOrangeUniformData = [ {name: "color", data: [1.0, 0.343, 0.277]} ];
  lineMat.shader.enable(gl);
  lineMat.shader.applyUniforms(gl, lineUniformData);
  lineMat.enable(gl);
  if(this.game.settings.grid) {
    lineMat.shader.applyUniforms(gl, lineWhiteUniformData);
    grid.grid.draw(gl, lineMat.shader);
  }
  lineMat.shader.applyUniforms(gl, lineRedUniformData);
  for(var i=0;i<grid.border.length;i++) {
    grid.border[i].draw(gl, lineMat.shader);
  }
  if(collision.floor) {
    lineMat.shader.applyUniforms(gl, lineBlueUniformData);
    for(var i=0;i<collision.floor.length;i++) {
      collision.floor[i].draw(gl, lineMat.shader);
    }
  }
  if(collision.wall) {
    lineMat.shader.applyUniforms(gl, lineYellowUniformData);
    for(var i=0;i<collision.wall.length;i++) {
      collision.wall[i].draw(gl, lineMat.shader);
    }
  }
  lineMat.shader.applyUniforms(gl, lineOrangeUniformData);
  for(var i=0;i<doodads.length;i++) {
    doodads[i].draw(gl, lineMat.shader);
  }
  for(var i=0;i<spawns.length;i++) {
    spawns[i].draw(gl, lineMat.shader);
  }
  if(grid.hover) {
    lineMat.shader.applyUniforms(gl, lineGreenUniformData);
    for(var i=0;i<grid.hover.length;i++) {
      grid.hover[i].draw(gl, lineMat.shader);
    }
  }
  if(selection) {
    lineMat.shader.applyUniforms(gl, lineGreenUniformData);
    for(var i=0;i<selection.length;i++) {
      selection[i].draw(gl, lineMat.shader);
    }
  }
  lineMat.shader.applyUniforms(gl, linePinkUniformData);
  if(this.game.cameraOriginScrollPoint) {
    grid.cammove.draw(gl, lineMat.shader);
  }
  lineMat.disable(gl);
  lineMat.shader.disable(gl);
