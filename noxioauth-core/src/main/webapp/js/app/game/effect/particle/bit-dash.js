...
function ParticleBitDash(game, pos, vel, colorA, colorB) {
...
ParticleBitDash.prototype.create = function() {
  ...
  var shockwave  = {
    ...
    properties: {pos: util.vec3.add(this.pos, util.vec3.make(0, 0, -0.125)), scale: 0.3, rotation: 0.0, color: util.vec4.copy3(colorA(), 0.55)}, tone: util.vec4.copy3(colorB(), 0.55)};
  ...
  for(var i=0;i<18;i++) {
    ...
    this.pushPart({
      ...
      properties: {pos: util.vec3.add(this.pos, util.vec3.scale(rand, 0.2)), vel: util.vec3.scale(reverse, 0.1), scale: 0.275, color: colorA(), tone: colorB(), rotation: -Math.atan(norm.y/norm.x)}
    });
  }
  ...
  for(var i=0;i<9;i++) {
    var rand = util.vec3.random();
    var speed = (Math.random()*0.075)+0.055;
    ...
    this.pushPart({
      ...
      properties: {pos: util.vec3.add(this.pos, util.vec3.scale(rand, 0.09)), scale: 0.1, vel: cmbvel, angle: axAng, color: colorA(), tone: colorB()}
    });
  }
};

ParticleBitDash.prototype.pushPart = Particle.prototype.pushPart;

ParticleBitDash.prototype.step = Particle.prototype.step;

ParticleBitDash.prototype.getDraw = function(geometry, decals, lights, bounds) {
...
ParticleBitDash.prototype.active = Particle.prototype.active;

/* Used by EffectDefinition.js */
ParticleBitDash.fxId = "particle";