...
/* Define Fire Hitstun Rainbow Particle System Class */
function ParticleBurnRainbow(game, pos, vel) {
  Particle.call(this, game, pos, vel);
}

ParticleBurnRainbow.prototype.create = function() {
  ...
  for(var i=0;i<12;i++) {
    this.pushPart({
      ...
      properties: {offset: util.vec3.scale(util.vec3.random(), 0.4), scale: 0.4, rotation: Math.random()*6.28, alpha: util.vec2.make(1,1), rbFrame: 0, frame: Math.floor(Math.random()*32), opacity: 2}
    });
  }
  ...
  for(var i=0;i<19;i++) {
    ...
    this.pushPart({
      ...
      properties: {pos: util.vec3.add(this.pos, util.vec3.scale(rand, 0.25)), scale: 0.165, vel: cmbvel, angle: axAng, alpha: util.vec2.make(1, 1), frame: parseInt(Math.random()*256)}
    });
  }
};

ParticleBurnRainbow.prototype.pushPart = Particle.prototype.pushPart;

ParticleBurnRainbow.prototype.step = Particle.prototype.step;

ParticleBurnRainbow.prototype.getDraw = function(geometry, decals, lights, bounds) {
...
ParticleBurnRainbow.prototype.active = Particle.prototype.active;

/* Used by EffectDefinition.js */
ParticleBurnRainbow.fxId = "particle";