...
/* Define Rest Hit Shockwave System Class */
function ParticleBlockShockwave(game, pos, vel, colorA, colorB) {
...
ParticleBlockShockwave.prototype.create = function() {
  ...
  for(var i=0;i<23;i++) {
    ...
    this.pushPart({
      ...
      spawn: function(pos, vel) {
        var rand = util.vec3.random();
        var speed = (Math.random()*0.135)+0.125;
      ...
      update: function() {
        ...
        this.properties.vel = util.vec3.scale(util.vec3.add(this.properties.vel, {x: 0.0, y: 0.0, z: -0.0095}), 0.955);
      ...
      properties: {pos: util.vec3.add(this.pos, util.vec3.scale(rand, 0.15)), scale: 0.175, vel: cmbvel, angle: axAng, color: colorA(), tone: colorB()}
    });
  }
};

ParticleBlockShockwave.prototype.pushPart = Particle.prototype.pushPart;

ParticleBlockShockwave.prototype.step = Particle.prototype.step;

ParticleBlockShockwave.prototype.getDraw = function(geometry, decals, lights, bounds) {
...
ParticleBlockShockwave.prototype.active = Particle.prototype.active;

/* Used by EffectDefinition.js */
ParticleBlockShockwave.fxId = "particle";