...
/* Define Crate Dash Rainbow Particle System Class */
function ParticleCrateDashRainbow(game, pos, vel) {
  Particle.call(this, game, pos, vel);
}

ParticleCrateDashRainbow.prototype.create = function() {
  ...
  var shockwaveMat = this.game.display.getMaterial("character.crate.effect.shockwaveRB");
  var flashMat = this.game.display.getMaterial("character.crate.effect.flashRB");
  var sparkMat = this.game.display.getMaterial("multi.hit.sparkRB");
  var blastMat = this.game.display.getMaterial("character.crate.effect.blastRB");
  ...
  this.pushPart({
    ...
    properties: {pos: util.vec3.add(this.pos, util.vec3.make(0,0,-0.245)), scale: 0.55, alpha: util.vec2.make(1,1), frame: Math.floor(Math.random()*256), rotation: 0.0}
  });
  ...
  this.pushPart({
    ...
    properties: {pos: util.vec3.add(this.pos, util.vec3.make(0,0,0.15)), scale: 0.35, alpha: util.vec2.make(1,1), frame: Math.floor(Math.random()*256), rotation: 0.0}
  });
  ...
  this.pushPart({
    ...
    properties: {offset: util.vec3.make(0,0,0.3), scale: 0.6, alpha: util.vec2.make(1,1), frame: Math.floor(Math.random()*256), rotation: 0.0}
  });
  ...
  for(var i=0;i<45;i++) {
    this.pushPart({
      ...
      properties: {pos: util.vec3.add(this.pos, util.vec3.scale(util.vec3.random(), 0.15)), vel: util.vec3.scale(util.vec3.inverse(util.vec3.normalize(this.vel)), 0.05+(Math.random()*0.025)), scale: 0.45, rotation: Math.random()*6.28, alpha: util.vec2.make(1,1), rbFrame: 0, frame: Math.floor(Math.random()*32), opacity: 2}
    });
  }
...
ParticleCrateDashRainbow.prototype.pushPart = Particle.prototype.pushPart;

ParticleCrateDashRainbow.prototype.step = Particle.prototype.step;

ParticleCrateDashRainbow.prototype.getDraw = function(geometry, decals, lights, bounds) {
...
ParticleCrateDashRainbow.prototype.active = Particle.prototype.active;

/* Used by EffectDefinition.js */
ParticleCrateDashRainbow.fxId = "particle";