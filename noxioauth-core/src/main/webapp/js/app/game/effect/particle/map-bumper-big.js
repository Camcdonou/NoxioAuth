...
function ParticleMapBumperBig(game, pos, vel, colorA, colorB) {
...
ParticleMapBumperBig.prototype.create = function() {
  ...
  var flash = {
    ...
    update: function(pos){
      ...
      this.properties.scale += 0.255;
      this.properties.color.w *= 0.875;
      this.properties.tone.w *= 0.825;
    },
    properties: {offset: util.vec3.make(0,0,0), scale: 2.2, rotation: 0, color: colorC(), tone: colorB()}
  };
  ...
  var waveA  = {
    ...
    length: 13,
    update: function(pos){
      this.properties.scale += 0.225;
      this.properties.rotation += 0.055;
      this.properties.color.w -= 0.75/this.length;
      this.properties.tone.w -= 0.75/this.length;
    },
    properties: {offset: util.vec3.make(0,0,-0.45), scale: 1.7, color: util.vec4.copy3(colorA(), 0.65), tone: util.vec4.copy3(colorB(), 0.85), rotation: Math.random()*6.4}
  };
  ...
  for(var i=0;i<49;i++) {
    ...
    rand = util.vec3.scale(util.vec3.normalize(rand), Math.random()*0.925 + 0.075);
    ...
    this.pushPart({
      ...
      update: function() {
        ...
        this.properties.vel = util.vec3.scale(this.properties.vel, 0.875);
        this.properties.color.w -= 1.0/this.max;
        this.properties.tone.w = this.properties.color.w;
      },
      properties: {pos: util.vec3.add(util.vec3.add(this.pos, rand), util.vec3.make(0, 0, -0.45)), vel: util.vec3.scale(util.vec3.normalize(util.vec3.make(rand.x, rand.y, 0)), 0.0085), scale: 0.335, rotation: Math.random()*6.28, color: colorA(), tone: colorB(), frame: Math.floor(Math.random()*32)}
    });
  }
  ...
  for(var i=0;i<45;i++) {
    ...
    this.pushPart({
      ...
      spawn: function(pos, vel) {
        ...
        var axAng = util.vec3.angle(up, cmbvel);
        this.properties.pos = util.vec3.add(pos, util.vec3.scale(rand, 0.95));
...
ParticleMapBumperBig.prototype.pushPart = Particle.prototype.pushPart;

ParticleMapBumperBig.prototype.step = Particle.prototype.step;

ParticleMapBumperBig.prototype.getDraw = function(geometry, decals, lights, bounds) {
...
ParticleMapBumperBig.prototype.active = Particle.prototype.active;

/* Used by EffectDefinition.js */
ParticleMapBumperBig.fxId = "particle";