...
/* Copied from http://www.labbookpages.co.uk/audio/javaWavFiles.html and modified */
/* @TODO: very messy and there are a lot of uneeded functions for writing and editing. clean out everything except decoding and validating */


/* Used for validation of a wav file */
...
public class WavFile
{
	...
	public static WavFile openWavData(byte[] data) throws IOException, WavFileException
	{
		// Instantiate new Wavfile and store the file reference
		WavFile wavFile = new WavFile();

		// Create a new file input stream for reading file data
		wavFile.iStream = new ByteArrayInputStream(data);
		...
		if (data.length != chunkSize+8) {
			throw new WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + data.length + ")");
		}
