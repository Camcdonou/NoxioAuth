...
public class AuthWebSocket extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession webSocket) {
      try {
        System.out.println("[AUTH-WEBSOCKET-DEBUG] Connection established: " + webSocket.getId());
        final NoxioSession session = dao.getUserDao().createSession(webSocket, dao);
        webSocket.getAttributes().put("session", session);

        System.out.println("[AUTH-WEBSOCKET-DEBUG] Scheduling PING task for: " + webSocket.getId());
        ...
        ScheduledFuture<?> pingTask = scheduler.scheduleAtFixedRate(() -> {
          try {
            System.out.println("[AUTH-WEBSOCKET-PING] Executing PING task for: " + webSocket.getId());
            if (webSocket.isOpen()) {
              webSocket.sendMessage(new PingMessage());
              System.out.println("[AUTH-WEBSOCKET-PING] Sent PING to: " + webSocket.getId());
              Oak.log(Oak.Type.SESSION, Oak.Level.INFO, "Sent WebSocket PING to keep connection alive: " + webSocket.getId());
            } else {
              System.out.println("[AUTH-WEBSOCKET-PING] WebSocket closed, skipping PING: " + webSocket.getId());
            }
          } catch (Exception e) {
            System.out.println("[AUTH-WEBSOCKET-PING] ERROR sending PING: " + e.getMessage());
            e.printStackTrace();
        ...
        System.out.println("[AUTH-WEBSOCKET-DEBUG] PING task scheduled successfully for: " + webSocket.getId());
      }
      catch(Exception ex) {
        System.out.println("[AUTH-WEBSOCKET-DEBUG] ERROR in afterConnectionEstablished: " + ex.getMessage());
        ex.printStackTrace();
